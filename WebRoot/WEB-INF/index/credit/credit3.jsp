...
<%--   <div class="clear pd10"><a href="#" target="_blank" class="blue_u f12">◆信用等级如何划分？</a><a href="#" target="_blank"  class="blue_u ml10 f12">◆为什么评价没有计分？</a><a href="#" target="_blank"  class="blue_u ml10 f12">◆什么是信用炒作？</a><a href="#" target="_blank"  class="blue_u ml10 f12">◆如何进行修改评价？</a><a href="#" target="_blank"  class="blue_u ml10 f12">◆有中差评怎么办？</a></div>--%>
  <div id="rightnav">
    <ul>
      ...
      <li class="normal_nav" onclick="javascript:window.location='${ctx}/user/trade/credit/userCredit2.shtml?userID=${user.id}&type=0'"><span>来自卖家的评价</span></li>
      <li class="action_nav" onclick="javascript:window.location='${ctx}/user/trade/credit/userCredit3.shtml?userID=${user.id}&isSeller=3'"><span>给他人的评价</span></li>
    </ul>
  </div>
  ...
  <div class="sale_u3 ml10">卖家 | 商品</div>
  <div class="clear"></div>
</div>
<!--list -->
<div class="sale_list">
	<s:iterator value="sellerAssess">
		<s:if test="grade==1">
			...
			<div class="sale_u3 ml10">
				<ul>
					<li>卖家：${user.username}　信用：${user.userInfo.buyerCredit}<span class="srank-buy-3">&nbsp;</span> </li>
...
<div class="manu">
	...
	<form id="pageForm" name="pageForm" action="${ctx}/user/trade/credit/userCredit3.shtml?userID=${user.id}&isSeller=3&grade=${grade}" method="post">
...
function ping(grade)
{
	window.location.href="${ctx}/user/trade/credit/userCredit3.shtml?&isSeller=3&userID=${user.id}&grade="+grade;
}
</script>
