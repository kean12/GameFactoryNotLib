...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="yhzx_rightside">
			...
			<div id="rightnav"><%--0 待付款|1 已付款|2 已发货3 交易关闭|4 退款处理|5 退款完成|6 交易成功--%>
				<ul>
					<s:if test="type==0">
		...
		<s:iterator value="orderList">
			<div class="sale_list">
				<div class="sale_u0">
					<span class="ml20 gray">编号：${orderNum}</span><span class="ml20 gray">下单时间：${orderTime}</span><span class="ml20 gray">卖家：${owner.username}</span>
				</div>
				<div class="sale_u3">
				...
				<div class="sale_u81">
					...
					<s:if test="buyType==1">
						<s:if test="state==1">
							<a style="cursor: pointer;" onclick="detail('${id}');" class="orange">已付款</a><br /><span class="orange">等待交易员发货</span>
						</s:if>
						<s:elseif test="state==2">
							<button class="abutton3" onclick="chk_state('${id}','6','${type}','${goPage}');">确认收货</button>
							<a style="cursor: pointer;" onclick="chk_state('${id}','4','${type}','${goPage}');" class="blue">申请退款</a>
						</s:elseif>
						<s:elseif test="state==3">
							<a style="cursor: pointer;" onclick="detail('${id}');" class="gray">交易关闭</a>
						</s:elseif>
						...
						<s:elseif test="state==7">
							<span class="orange">退款被拒绝</span>
							<button class="abutton3" onclick="chk_state('${id}','6','${type}','${goPage}');">确认收货</button>
						</s:elseif>
						<s:else>
							<button class="abutton2" onclick="chk_pay(${id});">付款</button>
						</s:else>
						<br />
						<a style="cursor: pointer;" onclick="window.location='${ctx}/user/trade/order/complaint.shtml?orderID=${id}'" class="blue">投诉</a><br />
						<a style="cursor: pointer;" onclick="detail('${id}');" class="blue">详情</a>
					</s:if>
					<s:else>
						<s:if test="state==1">
							<a style="cursor: pointer;" onclick="detail('${id}');" class="orange">已付款</a><br /><span class="orange">等待商家发货</span>
						</s:if>
						<s:elseif test="state==2">
							<button class="abutton3" onclick="chk_state('${id}','6','${type}','${goPage}');">确认收货</button>
							<a style="cursor: pointer;" onclick="chk_state('${id}','4','${type}','${goPage}');" class="blue">申请退款</a>
		...
		<%-- 分页  --%>
			<div class="manu">
				...
				<form name="pageForm" action="${ctx}/user/trade/order/list_bought_items.shtml" method="get">
