	...
	<body>
		...
		<s:iterator value="bizInfoList" status="index">
			<div class="sale_list" >
				...
				<div class="sale_u7">
					<a onclick="window.location='${ctx}/user/trade/bizInfo/top.shtml?bizInfoID=${id}'" class="red" style="cursor: pointer;">上架</a>|<a onclick="window.location='${ctx}/user/trade/bizInfo/storehouse_delete.shtml?bizInfoID=${id}'" class="red" style="cursor: pointer;">删除</a>
				</div>
				<div class="clear"></div>
			</div>
		</s:iterator>
			...
			<div class="manu" style="text-align:left; padding:5px 5px 3px">
				...
				<button class="delbutton ml10" onclick="chk_top();">上架</button>
			</div>
<script type="text/javascript">
	...
	function chk_delete(){
		if(confirm("删除后数据不能恢复，你确定要删除数据？")){
			document.store_form.action="${ctx}/user/trade/bizInfo/storehouse_delete.shtml";
			document.store_form.submit();
		}
	}
	function chk_top(){
		document.store_form.action="${ctx}/user/trade/bizInfo/top.shtml";
		document.store_form.submit();
	}
</script>
			<!--list end -->
